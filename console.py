...
class LiveConsole:
    """
    Rich console display for Ticket to Ride MCTS simulations.

    Provides a live updating interface showing progress
    and results of Monte Carlo Tree Search simulations.
    Disabled under PyPy for compatibility.
    """

    def __init__(self):
        """
        Initialise the live console interface.
        """

        # Skip initialisation if running under PyPy
        if is_pypy:
            self.enabled = False
            return
    ...
    def start_live(self, total_simulations=3000):
        """
        Start the live updating display for MCTS simulations.

        :param total_simulations: Expected total number of sims
        :type total_simulations: int
        """
        ...
        self.layout.split(self.layout_progress, self.layout_table)
    ...
    def update_display(self, sim_num, player_info):
        ...
        :type sim_num: int
        :param player_info: Dictionary containing player statistics.
        :type player_info: Dict[str, Any]
        """
        ...
        try:
            ...
            self.table.add_column(
                "Player", justify="center", style="cyan", no_wrap=True
            )
            self.table.add_column("Average Score", justify="center", style="magenta")
            self.table.add_column("Max Score", justify="center", style="magenta")
            self.table.add_column("Current Score", justify="center", style="magenta")
